...
class StorageService {
  ...
  static String encryptData(String data) {
    try {
      final wrappedData = jsonEncode({'version': 1, 'data': data});
  ...
  static Future<String> get _storagePath async {
    ...
    final newPath = '${directory.path}/passcode_data';
  ...
  static Future<bool> verifyFileIntegrity() async {
    try {
      ...
      if (await file.exists()) {
        ...
        jsonDecode(decryptedData); // Just to confirm if it can be parsed
        return true;
      }
      return true;
    } catch (e) {
  ...
  static Future<List<Credential>> loadCredentials() async {
    try {
      ...
      if (await file.exists()) {
        ...
        return jsonData.map((e) => Credential.fromJson(e)).toList();
      }
    } catch (e) {
